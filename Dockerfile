# Multi-stage Dockerfile para videoclub-backend (Java 25 + Spring Boot 4.1.1)
#
...
LABEL org.opencontainers.image.title="videoclub-backend" \
      org.opencontainers.image.description="Backend REST Resource Server for VideoClub" \
...
HEALTHCHECK --interval=30s --timeout=5s --retries=3 --start-period=30s \
    CMD curl -f http://localhost:${SERVER_PORT}/metrics/health || exit 1
...
LABEL org.opencontainers.image.title="videoclub-backend-native" \
      org.opencontainers.image.description="Backend REST Resource Server for VideoClub (GraalVM native image)" \
      org.opencontainers.image.authors="VideoClub UNRN"
...
COPY --from=native-build /app/target/videoclub-backend ./videoclub-backend
USER app
