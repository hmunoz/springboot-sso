...
2. **Token Relay in `videoclub-agent` (`:8085`)** — the agent is an OAuth2 Resource Server. The
   ...
   propagated to the two independent MCP domain services: `catalog-service` (`:8081`) and `membership-service` (`:8082`).
...
flowchart TD
    ...
    subgraph AgentSystem["videoclub-agent (:8085)"]
        ...
        Relay["TokenRelayService + McpTransportContext<br/>Relays user JWT across Reactor worker threads"]
        Supervisor --> SubAgents
        SubAgents --> Relay
    end

    Catalog["catalog-service (:8081)<br/>• catalog:8081/mcp<br/>• list_movies, get_movie, search_movies, create_movie<br/>• @PreAuthorize evaluates user permissions"]
    Membership["membership-service (:8082)<br/>• membership:8082/mcp<br/>• list_socios, get_socio<br/>• @PreAuthorize evaluates user permissions"]
    ...
    Relay -->|"POST /mcp (Bearer <user_jwt>)"| Catalog
    Relay -->|"POST /mcp (Bearer <user_jwt>)"| Membership
```
