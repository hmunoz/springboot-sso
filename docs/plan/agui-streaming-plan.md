...
**Scope:** `videoclub-agent` (:8085), `react-sso` (:5173), `catalog-service` (:8081), `membership-service` (:8082), gateway (:9500), Keycloak (:9090).
...
flowchart LR
    ...
    C["catalog-service :8081<br/>MCP server, @McpTool + @PreAuthorize"]
    M["membership-service :8082<br/>MCP server, @McpTool + @PreAuthorize"]
    K["Keycloak :9090"]

    B -->|"POST /api/agent/chat<br/>Bearer user JWT"| G
    G --> A
    A -->|"MCP Streamable HTTP<br/>Bearer user JWT (relay)"| C
    A -->|"MCP Streamable HTTP<br/>Bearer user JWT (relay)"| M
    B -.->|OIDC PKCE| K
    A -.->|JWKS| K
    C -.->|JWKS| K
    M -.->|JWKS| K
...
### 4.3 `springboot-sso` (Plataforma y Gateway)
...
- `docker/gateway/gateway.yml` — the `agent-service` route already matches `/api/agent/**`, so the
  ...
  `text/event-stream`.
- `docs/` — an ADR entry in `docs/adr-agente.md` for the AG-UI transport, plus a `docs/README.md` entry.
- No change to the two domain MCP microservices (`catalog-service` and `membership-service`), their tools, or `@PreAuthorize`. The authorization model is untouched;
...
[ADR-020](../adr-agente.md#adr-020-generative-ui-híbrido-con-extracción-y-validación-en-el-servidor), as [ADR-021](../adr-agente.md#adr-021-respuesta-bloqueante-en-vez-de-streaming-ag-ui-evaluado-y-diferido) already anticipated.

### The emission point already exists in our code

[ADR-024](../adr-agente.md#adr-024-el-artefacto-generative-ui-se-captura-antes-del-orquestador) moved Generative UI capture into `OrchestratorTools.captureArtifacts`, which runs
...
The blocker is still not the library. [ADR-014](../adr-agente.md#adr-014-token-relay-del-jwt-del-usuario-sin-fallback-a-la-service-account) binds identity to `SecurityContextHolder`,
