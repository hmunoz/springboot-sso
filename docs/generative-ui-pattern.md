...
sequenceDiagram
    ...
    participant CatalogMCP as catalog-service (:8081)
    ...
    SubAgent->>CatalogMCP: tool `search_movies(genre="SCIENCE_FICTION")`
    CatalogMCP-->>SubAgent: Lista de MovieDTOs JSON
...
Eliminarlo requiere que el artifact se construya desde la salida cruda de la tool — `TrackingToolCallback` ya intercepta cada invocación y es el lugar natural — de modo que el modelo escriba únicamente prosa. Queda como próxima iteración; ver también [`agui-streaming-plan.md`](plan/agui-streaming-plan.md), donde este campo `artifacts` mapea uno a uno a los eventos `STATE_SNAPSHOT` / `CUSTOM` de AG-UI.
