...
<@layout.emailLayout>
  ...
  <p style="margin: 0 0 16px 0; font-size: 15px; color: #4a5568; line-height: 1.6;">
    Recibimos una solicitud para restablecer las credenciales de acceso a tu cuenta en <strong>${realmName!'VideoClub SSO'}</strong>.
  </p>

  <p style="margin: 0 0 24px 0; font-size: 15px; color: #4a5568; line-height: 1.6;">
    Para generar una nueva contraseña segura, tocá el siguiente botón:
  </p>
  ...
  <div style="background-color: #f7fafc; border-left: 3px solid #3182ce; border-radius: 4px; padding: 12px 16px; margin: 24px 0;">
    <p style="margin: 0; font-size: 13px; color: #718096; line-height: 1.5;">
      ⏱️ Este enlace es válido por <strong>${linkExpirationFormatter(linkExpiration)}</strong>. Si no solicitaste este cambio, no te preocupes: tu contraseña actual sigue siendo válida y no se realizaron modificaciones.
    </p>
  </div>
